...
const getValidationSelection = (view: CommandContext['view']) => {
...
export const runValidations = async ({
  ...
  if (verb === 'get') {
    ...
    const result = await runQuery(ctx, { validation: { __args: { id }, ...selection } })
    if (result === undefined) return
    printNode({ node: result.validation, format: ctx.format, quiet: ctx.quiet })
    return
  }
  ...
  if (verb === 'list') {
    ...
    if (result === undefined) return
    printConnection({ connection: result.validations, format: ctx.format, quiet: ctx.quiet })
    return
  }
  ...
  if (verb === 'create') {
    ...
    if (result === undefined) return
    maybeFailOnUserErrors({ payload: result.validationCreate, failOnUserErrors: ctx.failOnUserErrors })
    if (ctx.quiet) return console.log(result.validationCreate?.validation?.id ?? '')
    printJson(result.validationCreate, ctx.format !== 'raw')
    return
  }
  ...
  if (verb === 'update') {
    ...
    if (result === undefined) return
    maybeFailOnUserErrors({ payload: result.validationUpdate, failOnUserErrors: ctx.failOnUserErrors })
    if (ctx.quiet) return console.log(result.validationUpdate?.validation?.id ?? '')
    printJson(result.validationUpdate, ctx.format !== 'raw')
    return
  }
  ...
  if (verb === 'delete') {
    ...
    if (result === undefined) return
    maybeFailOnUserErrors({ payload: result.validationDelete, failOnUserErrors: ctx.failOnUserErrors })
    if (ctx.quiet) return console.log(result.validationDelete?.deletedId ?? '')
    printJson(result.validationDelete, ctx.format !== 'raw')
    return
  }

  throw new CliError(`Unknown verb for validations: ${verb}`, 2)
}
