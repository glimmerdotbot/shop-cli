...
export const runPages = async ({
  ...
  if (verb === 'get') {
    ...
    const selection = applySelect(getPageSelection(ctx.view), args.select)

    const result = await runQuery(ctx, { page: { __args: { id }, ...selection } })
    if (result === undefined) return
    if (ctx.quiet) return console.log(result.page?.id ?? '')
  ...
  if (verb === 'list') {
    ...
    const nodeSelection = applySelect(getPageSelection(ctx.view), args.select)
